    ...
    if len(set(
        ...
        for key, value in component_data["sequences"].items():
            if len(value) < max_length:
                logging.info(
                    f"Stretching {key} of {component_data['scalars']['label']} to length of longest list. All sequences have to be of the same length.")
                component_data["sequences"][key] = value * (max_length // len(value))
